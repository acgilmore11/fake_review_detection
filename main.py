...
def main():
    ...
    # table = pd.concat([table, review_metadata(table), review_textual(table), reviewer_burst(table), max_num_reviews(table), percent_pos_reviews(table)], axis=1)
    table=pd.concat([table, review_metadata(table), review_textual(table), reviewer_burst(table), behavioral_features(table)], axis=1)
